...
set.seed(2019)
datagen <- function(num.pop, num.subc, k, Sigma, u2, u3, beta, lamzero, gam, theta) {
  # Data generation
...
calibration <- function(data, op1, op2, op3, op4) {
  ...
  if (op4==1) {
    ...
    dcal <- calibrate(dstrt, formula=make.formula(colnames(db.sum)), 
                      pop=c(`(Intercept)`=num.pop, colSums(db.sum)), 
...
# beta1 <- matrix(rep(0,nsim*9), ncol=9)
# beta2 <- matrix(rep(0,nsim*9), ncol=9)
# beta3 <- matrix(rep(0,nsim*9), ncol=9)
# beta4 <- matrix(rep(0,nsim*9), ncol=9)
# beta5 <- matrix(rep(0,nsim*9), ncol=9)
# time.elapse <- system.time(
#   for (i in 1:nsim) {
#     data <- datagen(1000,100,2,Sigma,0,0,beta,lamzero,gam,0.25)
#     # beta1[i,] <- calibration(data,1,1,1,1)
#     # beta2[i,] <- calibration(data,2,1,1,1)
#     # beta3[i,] <- calibration(data,1,2,1,1)
#     beta4[i,] <- calibration(data,1,1,2,1)
#     # beta5[i,] <- calibration(data,1,1,1,2)
#   }
# )
# result(beta1); result(beta2); result(beta3); result(beta4); result(beta5)

nsim = 2000
sim1 <- matrix(rep(0,nsim*9), ncol=9)
sim2 <- matrix(rep(0,nsim*9), ncol=9)
time.elapse <- system.time(
  for (i in 1:nsim) {
    data <- datagen(1000,100,2,Sigma,0,0,beta,lamzero,gam,0.25)
    sim1[i,] <- calibration(data,1,1,2,1)
    sim2[i,] <- calibration(data,1,1,2,2)
  }
)
result(sim1)
result(sim2)
